...
def test_chat_agent_unavailable():
    ...
    with patch(
        "playtomic_agent.api.create_playtomic_agent", side_effect=ValueError("Config missing")
    ):
...
def test_accepts_assistant_messages_without_role():
    ...
    mock_agent.stream.return_value = [{"model": {"messages": [DummyMsg(sample_text)]}}]
...
def test_chat_with_message_history():
    ...
    # Mock agent
    mock_agent = MagicMock()
    mock_agent.stream.return_value = [{"model": {"messages": [DummyMsg("Follow-up answer")]}}]
